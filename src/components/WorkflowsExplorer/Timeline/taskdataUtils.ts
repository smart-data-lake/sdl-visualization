...
export function timepointsOfTasks(tasks: Row[]): [number | null, number] {
  return tasks.reduce(
    (val, task) => {
...
export function getStepStatus(stepTaskData: Record<string, Row[]>): TaskStatus {
...
export function makeTasksForStep(currentData: Record<string, Row[]>, item: Row): Row[] {
