...
export function WorkspaceEmpty() {
  ...
  // Where the backend issues its own tokens, getting one is the first step, not an
  // aside: nothing creates a repository until SDLB uploads, and it needs the token.
  const issuesTokens = !!fetcher().capabilities?.().mcpTokens;
  ...
  // The hosted backend authenticates uploads with Cognito, so it has no token step.
  const tokenStep = issuesTokens
    ? `#### 1. Issue an access token

Go to [Settings, Access Token](#/${tenant}/settings/tokens) and issue one for the repository and
environment you are about to upload. Neither has to exist yet - this token is what lets SDLB
create them. Copy it, it is shown once, and put it in the job's \`SDLB_UI_TOKEN\` environment
variable.

`
    : "";
  // Numbered around the optional step above, so the guide does not skip a number.
  const step = (n: number) => (issuesTokens ? n + 1 : n);
...
${tokenStep}#### ${step(1)}. Add global.uiBackend configuration
...
#### ${step(2)}. Export SDLB configuration to the UI
