...
- useLineage: it holds the state of the lineage graph in the config explorer. `LineageProvider` provides two contexts: `useLineagePanel` (is the lineage panel open, and which element is it showing) and `useLineageGraph` (the toolbar settings: graph view, layout, expansion, node attribute filter)
