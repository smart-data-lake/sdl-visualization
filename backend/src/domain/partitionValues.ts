...
export function formatPartitionValue(value: any): string {
  // older state files wrap the map in `elements`, and normalizeStateFile only unwraps that form
...
export function formatPartitionValues(values: any[] | undefined): string {
  ...
  return (
    distinct.slice(0, MAX_PARTITION_VALUES).join(', ') +
    `, … (+${distinct.length - MAX_PARTITION_VALUES} more)`
  );
}
