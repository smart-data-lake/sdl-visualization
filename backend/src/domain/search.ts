import { ELEMENT_TYPES, type ElementType } from './types.js';
import type { SchemaColumn, SchemaData } from './types.js';

/**
 * The documents the global search indexes, and the MiniSearch options they are indexed with.
 *
 * Ported from src/util/ConfigExplorer/searchDocuments.ts of the frontend, and parity-tested
 * (test/unit/parity.test.ts) - change the pair together. It is deliberately pure:
 * no store access and no MiniSearch value import, so the rebuild service can import it
 * without pulling the library into the eager bundle.
 */
