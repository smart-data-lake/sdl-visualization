...
- **Six pieces of logic exist twice** — `src/util/WorkflowsExplorer/metrics.ts`, `src/util/WorkflowsExplorer/partitionValues.ts`, `src/util/ConfigExplorer/Graphs.ts`, the filters in `src/util/ConfigExplorer/ConfigData.ts` plus `getPropertyByPathIgnoreCase`, `updateStateFile`/`endAnchorOf` in `Attempt.ts`, and `build_index.py`'s `getRuns()` — copied into `backend/src/domain/`. `backend/test/unit/parity.test.ts` imports the frontend originals and asserts both agree over the getting-started fixture, so drift fails a test rather than a user. **Change the pair together.**
