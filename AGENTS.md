...
yarn build:search-index # global search index → public/search/index.json (build_index.sh runs it too)
...
Lint uses ESLint 9 flat config (`eslint.config.js`), rebuilt from the maintained plugins after the unmaintained CRA `react-app` config was dropped. Rules with pre-existing violations are set to `warn` and annotated with their count; `yarn lint` fails on errors only, so those 114 warnings are a visible cleanup backlog rather than a blocker.

Local data setup (needed before anything is visible): put config files in `public/config` and state files in `public/state`, then `./build_index.sh <path-to-statefiles> <path-to-configfiles>` (Python 3; creates a venv, writes JSON-Lines `index` files, then builds the search index with Node). `backendConfig` in `public/manifest.json` must be `local;`.
...
- `Graphs.ts` is the graph domain model: `Node`/`Edge` base classes, `DataObject`/`ActionObject` (both are *nodes* in the newer separated model), `DAGraph` with traversal/level computation (`returnDirectNeighbours`, `returnPartialGraphInputs`, `getSubGraph`, `shortestPathToAny`), and the dagre layout helpers (`dagreLayout`, `dagreLayoutRf`). This is the file `tests/graph.test.ts` covers. The lineage graph's own layout is `LineageLayout.ts` (`tests/lineageLayout.test.ts`), see below.
...
The ReactFlow instance is **not** in that state — components get it from `useReactFlow()`, and the functions in `LineageTabUtils.tsx`/`Graphs.ts` take it as a parameter, together with the graph settings they need (`lineageGraphState`). The grouping working data (connected components / subgroups and their ReactFlow counterparts) is module-level mutable state inside `LineageTabUtils.tsx`, because only imperative code there reads and writes it, and so is the layout model cache in `LineageLayout.ts` — a `WeakMap` per `DAGraph`, for the same reason plus one more: every node of the graph consumes the toolbar context, so rebuilding the model there would re-render all of them.

The **layout is stable by construction**: `LineageLayout.ts` gives every node of a view a (rank, order) placement, computed once per graph and direction from sorted input, and `assignCoordinates` derives the coordinates from that plus the nodes' declared sizes. Nothing minimises crossings per interaction any more, so no node can change its order because another one appeared or was selected; selecting an element highlights it and grows the graph around it instead of re-creating the flow. `dagreLayoutRf` survives for the grouping-box path, which owns its children's coordinates. See `src/components/ConfigExplorer/LineageTab/README.md`.

`ConfigData` builds a fourth graph next to those three: `relationsGraph` (`src/util/ConfigExplorer/RelationsGraph.ts`), data objects as nodes and their declared `table.foreignKeys` as edges. It is the Entity Relation Diagram, selected as a fourth graph view in the toolbar, and it is the one graph that is **not acyclic** — foreign keys point in circles, which is why the traversals in `Graphs.ts` carry a visited set. A foreign key names the data object it references (`dataObjectId`); a key in the pre 3.x `db`/`table` form is ignored. The Foreign Keys accordion of the configuration view shows that reference as a chip leading to the referenced data object, greyed out and inert where the configuration does not describe it; the Schema tab marks the same keys per column, with the same key symbol (`PrimaryKeyIcon`) and the same chips, in two columns it only offers where the configuration declares such a key (`SchemaTab.tsx`) - `PK` first, `FK` hidden until the column selection menu above the table turns it on. A data object node can show its columns, marking primary and foreign keys, in every view; the columns are the configured keys merged with the exported schema (`ColumnModel.ts`), opened through a three step control along the node's lower border (none, key columns, all columns), and a relation is drawn onto the columns themselves once a node is opened. `src/components/ConfigExplorer/LineageTab/README.md` has the details, including the two rules that are easy to break around ReactFlow handles.
...
`Workflows` (list) → `WorkflowHistory` (runs + charts) → `Run` (single attempt, tabs: timeline / table / lineage). SDLB state files are normalized in `Attempt.ts` — `updateStateFile` migrates older state-file formats (flattening `results[].subFeed`, `mainMetrics` → `metrics`, `inputIds`/`outputIds` objects → ids), so new state-file format changes belong there. `Attempt` also fixes the **row order** used by the timeline and table: exec start, then init start, then prepare start (`compareMultiFunc`), because SDLB starts every ready action of a stage in the same millisecond and the later keys break those ties (`tests/timelineSort.test.ts`). It also derives the attempt's **end anchor** (`endAnchorOf`): for a final attempt (`isFinal`), a phase that recorded no end timestamp is measured up to the last timestamp the state file has evidence for, not up to now — otherwise a cancelled action makes a long-finished run look like it is still going. A live attempt has no anchor, so its phases keep counting up (`tests/timelineEndAnchor.test.ts`). `types.ts` holds the run/row domain model (`Row`, `TaskStatus`, `StateFile`) and derives durations from ISO-8601 strings. The timeline (`src/components/WorkflowsExplorer/Timeline/`) is a virtualized Gantt-style view adapted from Netflix Metaflow (see `Timeline/LICENSE`, which must be retained, and that folder's `README.md`). It has since been de-Metaflowed and moved onto MUI Joy; status/phase colours live in `src/util/WorkflowsExplorer/statusColors.ts` because the history chart, status icons and toolbar share them. `src/util/WorkflowsExplorer/partitionValues.ts` spells the partition values an action selected (`results[].partitionValues`) for the run tables' optional, hidden by default "Selected Partition Values" column; the workflow run table's value is the first action in the DAG that selected any, precalculated when the attempt is indexed (`actionsInDagOrder` in `backend/src/domain/stateFile.ts` and its mirror in `build_index.py`), because the index carries no `inputIds` to reconstruct the DAG from. The config explorer's "Last 5 runs" panel shows the same per element (issue #133) - an action's selected partition values and incremental state (`dataObjectsState`), a data object's written partition values and output metric - by loading those runs' state files (`useFetchRunsQuiet`, keyed like `useFetchRun`) and reading them with `src/util/WorkflowsExplorer/elementRunDetails.ts`; nothing is added to the index. `src/util/WorkflowsExplorer/phases.ts` maps phase names (Prepare/Init/Exec) onto the state file's timestamp fields and is the single source for both the bars and the visible time window — the timeline auto-zooms to the range the selected phases actually cover, so changing the phase filter re-fits instead of leaving empty space.

### Global search (`src/components/Search`, `src/util/ConfigExplorer/searchDocuments.ts`)

The search box in the title bar (`src/layouts/Header.tsx`) searches one **configuration version**: its elements, the description documents and the schema columns with their descriptions. Workflow runs and state files are deliberately out of scope (issue #15). It is a ⌘K/Ctrl+K palette rather than a dropdown because the header sits at `zIndex: 10000`, above every portalled Joy popup — `Authentication.tsx` already carries a manual override for the same reason.

**Searching happens in the browser**, over a [MiniSearch](https://github.com/lucaong/minisearch) index fetched once per version; there is no search endpoint. `searchDocuments.ts` defines the documents and the options and is the seventh mirrored pair (`backend/src/domain/search.ts`). The options come in two halves on purpose: `SEARCH_INDEX_OPTIONS` must be identical to what the index was built with, because `MiniSearch.loadJS` fails **silently** on a mismatch — hence `SEARCH_SCHEMA_VERSION`, written into the index metadata and refused when it differs — while `SEARCH_QUERY_OPTIONS` is query-time only, since `boostDocument` is a function and reads `kind` back out of `storeFields`.

The index is built **only by an explicit rebuild**: `POST /api/v1/search/index` on the backend, or `yarn build:search-index` (`scripts/buildSearchIndex.ts`) for a statically served project, which reads `public/` and writes `public/search/index.json`. `build_index.sh` calls it, non-fatally. So **no index is a normal state**, and the search falls back to indexing the configuration in the browser — elements only, because descriptions and columns live in per-element files a query cannot fetch one by one — and the palette's footer says so.

A query may start with a known `<scope>:` prefix to narrow itself — by document kind (`do:`, `action:`, `connection:`, `description:`, `column:`, `element:`) or by field (`id:`, `name:`, `type:`, `layer:`, `area:`, `feed:`, `tag:`, `body:`). `parseQuery` consumes only those words, so a colon inside a URL or a qualified table name is searched as typed; the rest becomes MiniSearch's `filter` or `fields`.

Two things that are easy to break. **Every query the search makes must be a quiet one**: `handleError` in `useFetchData.tsx` rethrows into the `ErrorBoundary`, which is the root route's `errorElement`, so anything thrown from a hook used in the header replaces the whole page (hence `useFetchSearchIndex`, `useFetchConfigQuiet`, `useFetchConfigVersionsQuiet`, and `parseSearchIndex` refusing rather than throwing). And **a column hit carries the schema export it was indexed from** (`?column=…&tstamp=…`), because the newest export of a data object is frequently a failed one carrying an error message and no columns — the builder takes the newest export that actually has columns, and the deep link has to select that same one. `src/components/Search/README.md` has the details.

The selected configuration version lives in `useConfigVersion` (`src/hooks/useConfigVersion.tsx`), mounted above `<Routes>` in `App.tsx`, because the header is outside the config explorer's route subtree — and as a side effect the version now survives navigating to the workflows explorer and back.
...
- **Seven pieces of logic exist twice** — `src/util/WorkflowsExplorer/metrics.ts`, `src/util/WorkflowsExplorer/partitionValues.ts`, `src/util/ConfigExplorer/Graphs.ts`, the filters in `src/util/ConfigExplorer/ConfigData.ts` plus `getPropertyByPathIgnoreCase`, `updateStateFile`/`endAnchorOf` in `Attempt.ts`, `build_index.py`'s `getRuns()`, and `src/util/ConfigExplorer/searchDocuments.ts` — copied into `backend/src/domain/`. `backend/test/unit/parity.test.ts` imports the frontend originals and asserts both agree over the getting-started fixture, so drift fails a test rather than a user. **Change the pair together.**
